...
def shuffle_and_pair_2(conv, fourier):
    """
    Shuffle pieces and return packed embeddings, permutation target, and one-hot target.
    ...
    Returns:
        packed:  (N, 2D) tensor — shuffled, all three embeddings concatenated per piece
        target:  (N,)   tensor — original indices (permutation)
        one_hot: (N, N) tensor — one-hot encoding of target
    """
    N = conv.shape[0]
    ...
    packed = np.concatenate([conv[perm], fourier[perm]], axis=1)  # (N, 3D)
...
class PuzzleDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return conv_emb, fourier_emb


def collate_fn(batch):
    # batch = list of (conv_emb, fourier_emb, color_emb) tuples
    packed_list, one_hot_list = [], []
    for conv, fourier in batch:
        packed, _, one_hot = shuffle_and_pair_2(conv, fourier)
